...
#[derive(Snafu, Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
...
pub enum ParseNzbError {
    ...
    #[snafu(display("Invalid or missing 'groups' element within the 'file' element. Each 'file' element must contain at least one valid 'groups' element."))]
    GroupsElement,
    ...
    #[snafu(display("Invalid or missing 'segments' element within the 'file' element. Each 'file' element must contain at least one valid 'segments' element."))]
    SegmentsElement,
    ...
    #[snafu(display("Invalid or missing 'file' element in the NZB document. The NZB document must contain at least one valid 'file' element, and each 'file' must have at least one valid 'groups' and 'segments' element."))]
    FileElement,

    /// Indicates an invalid or missing required attribute '{attribute}' in a 'file' element.
    #[snafu(display("Invalid or missing required attribute '{attribute}' in a 'file' element."))]
    FileAttribute {
    ...
    #[snafu(display("The NZB document is not valid XML and could not be parsed: {message}"))]
    XmlSyntax {
...
pub enum ParseNzbFileError {
    ...
    #[snafu(display("I/O error while accessing file '{}': {}", file.display(), source))]
    Io {
    ...
    #[snafu(display("Gzip decompression error for file '{}': {}", file.display(), source))]
    Gzip {
    ...
    #[snafu(display("NZB parsing error: {}", source))]
    Parse {
